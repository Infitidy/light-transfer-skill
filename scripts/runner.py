...
# 导入 RunningHub 标准技能脚本
# 正确路径：<workspace>/skills/OpenClaw_RH_Skills/runninghub/scripts
# __file__ 位于: <workspace>/光线迁移/scripts/runner.py
workspace_root = Path(__file__).parents[2]  # <workspace>
skill_scripts_path = workspace_root / "skills/OpenClaw_RH_Skills/runninghub/scripts"
...
try:
    ...
    logger.error(f"尝试路径: {skill_scripts_path}")
    logger.error("请确保 OpenClaw_RH_Skills 仓库已克隆到 skills/ 目录")
    raise
...
class RunningHubRunner:
    ...
    def run_one(self, pic1_path: str, pic2_path: str, output_name: str) -> Optional[Path]:
        ...
        try:
            ...
            logger.error(f"❌ 任务失败: {output_name} - {e}")
            return None
