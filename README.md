...
- **操作系统**: Ubuntu 20.04+ (推荐) / Debian 系 Linux
- **Python**: 3.10 或更高版本
- **必需工具**: `curl` 命令（文件上传）, `git`
- **磁盘空间**: 建议 10GB 以上可用空间（缓存+输出）
- **网络**: 可访问 RunningHub API (https://www.runninghub.cn)

> ⚠️ **注意**: 本 Skill 开发和测试环境为 **Ubuntu 22.04**。其他平台（如 macOS/Windows）理论上可运行，但未做兼容性验证，如有问题请反馈。
